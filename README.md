...
- следующий узкий milestone: one-shot Tele2 polling command (`poll-once`) с durable dedup, без scheduler
...
- `TELE2_POLL_LOOKBACK_MINUTES=180`
- `TELE2_POLL_FETCH_LIMIT=30`
- `TELE2_POLL_MAX_CANDIDATES=5`
...
- obtain real Tele2 payload samples and confirm `TELE2_*_FIELD_PATH`
- run controlled dry-run via `POST /api/ingest/tele2` before any live switch
