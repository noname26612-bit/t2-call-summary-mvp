...
- `Telegram message format v2` завершён как отдельный узкий workstream
- текущий активный узкий workstream: post-incident hardening (`Tele2 poller auth/env + ops access baseline`)
- остальные улучшения (ignored numbers / owner routing / buttons / poll interval / missed-call filtering) остаются вне scope

## Telegram message format v2 (completed workstream)
...
Что хотели: Краткое тезисное резюме (2-4 строки, без воды).
...
Сценарий: Запчасти
...
- обязательные поля: `Кто звонил`, `Когда звонил`, `Что хотели`, `Сценарий`
...
- `Компания` печатается только если явно прозвучала в разговоре
- `Номер заказа` печатается только если явно прозвучал в разговоре
...
- keep `Telegram message format v2` as completed baseline
- roll out post-incident hardening changes for poller env/auth signaling
...
- keep ignored numbers / owner routing / Telegram buttons / poll interval unchanged in this wave
