...
- текущий узкий репозиторный pass: Telegram callback polling via `getUpdates` for transcript button flow (без public webhook requirement)
...
## Telegram callback updates via `getUpdates` (active narrow pass)
...
- deploy callback polling (`getUpdates`) pass to production main app and run one fresh live verification
