...
- следующий узкий milestone: выкатить и подтвердить token refresh + log rotation hardening на VM (без webhook/cutover)
...
- verify automatic token refresh on VM (preflight + retry-after-403)
- keep `/opt/t2-call-summary/tele2-poll.env` writable only for service user (`0600`)
- install and verify logrotate for `/home/artem266/t2-call-summary-mvp/logs/tele2-poll-once.log`
- monitor scheduler + refresh logs for 1-2 days after enablement
