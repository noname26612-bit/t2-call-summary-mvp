...
- текущий узкий репозиторный pass: transcript storage + Telegram button `Транскрипт (.txt)`
...
## Transcript storage + Telegram `.txt` button (active narrow pass)
...
- deploy transcript-button pass to production main app and run one fresh live verification
...
- `POST /api/telegram/webhook` — callback endpoint для inline button `Транскрипт (.txt)`
